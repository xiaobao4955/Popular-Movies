...
package com.burhan.udacity.popularmovies.utils;
...
public final class UiUtils {
    ...
    public static String joinGenres(List<Genre> genres, String delimiter, @NonNull StringBuilder builder) {
        Log.d(TAG, "joinGenres() called with: genres = [" + genres + "], delimiter = [" + delimiter + "], builder = [" + builder + "]");
        builder.setLength(0);
        if (!Lists.isEmpty(genres))
            for (Genre genre : genres) {
                if(genre==null) continue;
                if (builder.length() > 0) builder.append(delimiter);
                if (genre.getName() != null) builder.append(genre.getName());
            }
        return builder.toString();
    }
}
